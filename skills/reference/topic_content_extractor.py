"""
New pipeline stage: for each reading, extract concrete, topic-organized content -- NOT how it
relates to the instructor's book or CoRE Stack (that's book_alignment_scorer.py's separate,
still-independent job). Produces slide-ready facts only (key_facts/mechanism/data_points).

Longer-form extended notes are NOT produced here anymore (v3 change) -- that's
appendix_writer.py's job, run on demand after a lecture deck exists and knows specifically what
it needs elaborated. This keeps reduce-1 (this script) cheap and blind-appendix-guessing out of
the pipeline; appendix_writer.py reuses this script's map_candidates.json cache rather than
re-extracting anything.

Two-phase map-reduce, independently backend-selectable:

  MAP (per section/chunk, topic-agnostic): "pull out concrete facts/mechanisms/numbers from this
  section" -- deliberately dumb-and-narrow, doesn't try to sort into topics yet. Meant to absorb
  bulk, cheap, parallelizable work -- the natural fit for local Ollama.

  REDUCE (per reading x topic): takes the deduped candidates from all of that reading's chunks,
  filtered to what's relevant, and writes the final key_facts/mechanism/data_points for that
  (reading, topic) pair. Small input by this point, benefits from a stronger model.

Install: pip install anthropic requests

Usage:
    python skills/topic_content_extractor.py --out-dir data/lecture-prep/<example-id>
    python skills/topic_content_extractor.py --out-dir data/lecture-prep/<example-id> \
        --map-backend ollama --reduce-backend anthropic

Backend resolution: --map-backend / --reduce-backend flags, else MAP_BACKEND / REDUCE_BACKEND
env vars, else ALIGNMENT_BACKEND from .env (get_alignment_backend()) as a shared fallback. These
two env vars aren't wired into src/llm_config.py yet -- read directly from os.environ here; worth
formalizing there later if this split proves useful long-term.

Writes:
    <out-dir>/map_candidates.json   -- per-reading map-phase cache (chunk-level candidates) --
                                        reused by appendix_writer.py, don't delete between runs
    <out-dir>/topic_content.json    -- per-(reading, topic) reduce-phase cache (key_facts/
                                        mechanism/data_points only) -- this is the artifact
                                        run_lecture_prep.py reads to build the slide manifest
"""
import os
import re
import json
...
MAP_MAX_TOKENS = 1200
REDUCE_MAX_TOKENS = 1200  # v3: lowered from 4000 -- appendix (the long field) moved to
                          # appendix_writer.py, so reduce-1's output is just 3 short fields now
MAX_CHUNK_CHARS = 8000  # guard against one oversized section blowing the map prompt
...
def resolve_backend(cli_value, env_var_name):
    ...
    return get_alignment_backend().lower()
...
def extract_json_array(raw):
    parsed = extract_json_object(raw)  # in case it already returns parseable structures
...
# ============================================================================
# dedup (same fuzzy word-overlap approach used elsewhere in this pipeline)
# ============================================================================
...
def run_map_phase(reading_id, reading, entry, map_cache, backend, anthropic_client, model_name,
                  force_map):
    """Returns (deduped_candidates, used_fallback_abstract: bool). deduped_candidates is None if
    there's truly nothing to work with (no PDF text and no abstract)."""
    text_md_path = entry.get("text_md_path")
    chunks = get_section_chunks(text_md_path) if text_md_path else []

    if not chunks:
        # No usable PDF markdown (video, still-needs-download, or markdown extraction failed) --
        # fall back to the abstract as a single pseudo-candidate, skip map LLM calls entirely.
...
def main():
    ...
    map_backend = resolve_backend(args.map_backend, "MAP_BACKEND")
    reduce_backend = resolve_backend(args.reduce_backend, "REDUCE_BACKEND")
    ...
    for entry in manifest["readings"]:
        ...
        save_json_cache(map_cache_path, map_cache)  # persist after each reading, not just at exit
        ...
        for topic_id in topics:
            ...
            save_json_cache(topic_cache_path, topic_cache)  # persist after each topic
