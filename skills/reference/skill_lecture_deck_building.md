...
on-demand step run *after* the deck exists, driven by exactly these requests, not an automatic
...
## End-to-end workflow (v3, formalized)

The full process now runs in four handoffs, alternating between local execution (instructor's
machine, API/Ollama access) and chat-based work (me):

1. **Instructor generates the slide-facts manifest locally**: `run_lecture_prep.py` (resolve →
   extract → align → `topic_content_extractor.py`) produces `lecture_prep_manifest.json`. No
   appendix content at this stage.
2. **I build the deck and outline** from that manifest, following this skill file, and produce
   `outline.md` containing the slide-by-slide table plus a `## Appendix Requests` block for
   whatever I flagged as needing more depth than a slide can hold.
3. **Instructor generates the appendix locally**: `appendix_writer.py --outline outline.md`
   reuses the cached map-phase candidates (no PDF re-extraction) and writes targeted
   `appendix.md` sections for exactly those requests.
4. **I sharpen the deck** using the now-available `appendix.md` -- pulling specific mechanisms,
   thresholds, and numbers into the slides that requested them, updating speaker notes to cite
   `appendix.md` as the source, and rebuilding.

**QA discipline for step 4, learned from doing this once already**: don't trust the appendix
blindly just because it exists.
- **Check every requested section actually has content.** A known failure mode (seen twice, on
  two different requests, across two separate appendix-generation runs) is a request producing a
  header with zero content underneath -- silent generation failure, not something `git diff`-style
  skimming catches easily. `grep` for headers followed by only blank lines before trusting an
  appendix file is complete.
- **Watch for near-identical definitions given to named-differently things** -- e.g. two distinct
  technical indices described with the same underlying formula is a strong signal of model
  confabulation (padding out something it lacks real source grounding for), not a genuine
  coincidence. When this happens, don't put the specific numeric claim on a slide; either omit it,
  hedge it explicitly ("the paper isn't fully consistent on this exact threshold"), or flag it in
  speaker notes as needing verification against the source PDF directly.
...
4. Deliver the `.pptx` plus a short outline summary (topic-by-topic slide list with figure
   requests called out) in the same response -- don't make the instructor open the file to find
   out what's in it before deciding whether to review further.
